...
def update_word_pattern(word, pattern, letter):
        where_found = [i for i, c in enumerate(word) if c == letter] #find all location of the letter
        if len(where_found) !=0:
            #add letter to pattern in places from where_found indexes list
            pos = 0
            while pos < len(where_found):
